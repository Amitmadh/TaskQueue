...
class Backend(Protocol):
    ...
    async def claim(self) -> dict[str, Any]:
        """Block until a job is available, lease it, and return its record.
    ...
    async def take_result(self, job_id: str) -> dict[str, Any]:
        ...
        the terminal record, and removes it from the store. This consuming read
        maps onto Redis `GETDEL`, so a networked backend delivers the result and
        frees it in one round-trip. Single-consumer: the record is gone
        afterward. Raises if the job is unknown.
        """
        ...
