...
How TaskQueue is put together, and why each piece is shaped the way it is. Most of the interesting decisions are about *races*: what two processes
...
cannot be expressed. You would have to read, decide in Python, and write
hopefully, with the whole race back in the gap. Redis runs a script atomically, so every guard
...
`test_two_worker_processes_split_jobs` is the proof that fakeredis cannot give:
it lives inside the test process, so it can say nothing about atomicity
*between* processes.
...
- **A worker killed between them** leaves the id on its processing list with the
  ...
  redelivered; the status reset is skipped for that one.
- **`attempts` is counted by the claim script rather than by the reaper.** Counting a lost
...
- **An error on the second round trip used to keep the lease forever.** The worker
  logs and claims again, and it stays alive and beating, which is what keeps
  every reaper away (see below). `claim` now returns the id to the queue in
...
script, so two reapers racing cost a wasted round trip rather than a duplicated job. A lock
...
- **A worker registers before it can claim.** `Worker.__aenter__` `await`s one
  ...
  claim won by 4-17 ms. A `SIGKILL` in that window strands the job for good,
  because `reap` walks expired **members** and nothing else: a lease whose owner
  is not in the zset is invisible to every reaper.
  `test_a_worker_registers_before_it_can_claim`.
- **A pool deregisters last.** `__aexit__` cancels the claim loops and waits for
  them (each one `release`s its in-flight lease on the way out), and only then
  stops the upkeep loop. Stopping upkeep first would let this worker's beat lapse while it is
...
forever, so the wait re-reads the durable state every `_NOTIFY_POLL_SECONDS`
(5s). Five seconds is a backstop rather than a mechanism: one `HGET` per waiting
job per interval, forever, against a few seconds of extra latency in a failure
...
The worker side is two sibling tasks, the job coroutine and a `wait_cancel`
watcher, raced with `asyncio.wait(FIRST_COMPLETED)`:
...
- **A missing record means the job finished, not that it was cancelled.** The poll's
  ...
  the job settles.
- **A watcher can fail as well as fire.** `asyncio.wait` reports
...
- **Fail-fast fans out from the waiter rather than from `__aexit__`.** A done-callback on
...
visible on the wire as well as in the producer's memory.
...
never finishes, with nothing in the log.
...
the first set is still running, owned by nobody. Measured before the fix, by
releasing a nested parent's lease, which is what a reaper does after its worker
is killed:
...
in a `ContextVar` before it creates the task (a task copies the context at
*creation*, so anything set afterwards is invisible to the body), and
...
`next_spawn` counts per **body** rather than per scope. A scope's id is a fresh `uuid4`
...
workflow: that a re-run issues the same sequence of spawns. A spawn with no job
in context, such as a producer script, returns `None` and `Job` falls back to
`uuid4`, so two identical spawns from a script still get distinct ids.
...
`MULTI` cannot branch. Skipping the record write but still pushing the id would
be worse than no guard at all, since the job would then be claimed twice, so the
test asserts through `claim` rather than through the record. `tests/test_child_ids.py` covers both halves and
...
single-consumer, so one of them waits forever. The reaper only redelivers a job
whose worker is already gone, so the paths that exist today do not produce that.
A retry that fires while the first attempt is still running would, which is a
constraint on retries rather than a solved problem.
...
inferring its liveness from a heartbeat, and, as in the reaper section above, a
beat that goes quiet because a process is busy looks the same as one that went
quiet because it died. The cost of getting that wrong is cancelling work that is still
