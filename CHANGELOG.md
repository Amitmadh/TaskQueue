...
- **A task body that runs twice no longer forks its subtree.** At-least-once delivery
  ...
  - `TaskQueue.context` holds a `JobContext` in a `ContextVar`, set by
    ...
    counter per **body** rather than per scope: a scope's id is a fresh `uuid4` on every
...
- **Waiting on a result no longer costs a connection.** `take_result` and `wait_cancel`
  each opened their own pubsub, so N jobs awaited at once held N connections for the
  length of the wait, and redis-py's default pool of 100 made that a ceiling on fan-out:
  `gather` over 100 handles failed with `MaxConnectionsError` against fakeredis and a
  real server alike. A `_Notifier` now carries every channel on one connection, with a
  single reader task fanning each message out to that channel's waiters; subscribing on
  the first watcher, unsubscribing on the last. Measured after: 60 waiters share a pool
  of **8**, and 300 run against fakeredis's default 100.
  - It exists only while somebody is waiting. The first watcher opens the connection and
    starts the reader; the last one to leave stops the reader and closes the connection,
    so the `Backend` Protocol did not need a shutdown hook and an idle backend holds
    nothing. The release is shielded from cancellation, because a waiter is normally
    cancelled rather than finished (a scope tearing its children down, a deadline),
    and unshielded cleanup would be cancelled with it, stranding reader and connection.
  - Neither loop trusts the message any more. Both re-read the durable record on every
    wake and exit only on what it says, so a message means "look again" rather than
    "you are done"; the poll backup is now the same code path as a notification.
  - This does not make the pool irrelevant. A burst of concurrent *commands* can still
    exceed a small one, and redis-py's default pool raises rather than waits, so very
    wide fan-out still wants `BlockingConnectionPool` or a larger `max_connections`.
    What changed is that waiting itself no longer consumes a connection.
- **`tests/test_base.py::test_many_waiters_do_not_each_hold_a_connection`** pins it: 60
  waiters, `max_connections=8`. Restoring the per-waiter pubsub fails it in 10s with
  `ConnectionError: No connection available`, because the waiters take every connection
  and the workers that would finish their jobs cannot get one.
- **A watcher is registered before its channel is subscribed.** `subscribe`
  is the only await in `join`, and a waiter is normally cancelled rather than finished;
  registering after it meant a cancellation there returned no event to the caller, so
  nothing ever called `leave` and the channel stayed in the table with its connection
  open. Intermittent by nature (one run in three of a 60-job fan-out on a pool of 8),
  so `test_a_cancelled_subscribe_leaves_no_channel_behind` provokes it directly rather
  than waiting for the race.
...
- **A poll backup behind both pub/sub waits, so a dropped notification costs latency
  ...
  `_NOTIFY_POLL_SECONDS` (5s), since the stored record is authoritative and the message
  only says that something changed. Five seconds is a backstop rather than a
  mechanism: one `HGET` per waiting job per
...
- **A failure now cancels its siblings immediately rather than at the end of the block.** The
...
- **A failed `release` on shutdown swallowed the worker loop's own cancellation.**
  `release` raises `KeyError` for a record that no longer exists (the state left
...
- **`RedisBackend`: the whole `Backend` Protocol, against real Redis.** A hash per job,
  ...
  than a pipeline, because `MULTI` cannot branch (`pipe.hget()` returns the pipeline,
  not a value), so "if the job still exists and is not terminal, then write" is one atomic
...
- **A task name is now treated as a wire identifier rather than a Python detail.** It is
...
- **`Job.to_record` now produces a Redis-shaped record.** Control fields are plain
  ...
  and result are `bytes` blobs, because a Redis hash stores strings and bytes, never
...
- **`Backend.claim()` returns `dict | None`** and must never block indefinitely. It used
  ...
  observe a shutdown request, so `drain()` waited forever on a pool that was, by definition,
